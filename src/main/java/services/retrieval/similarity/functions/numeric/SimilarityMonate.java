...
import models.datatypes.custom.MonateValue;
 ...
 */
public class SimilarityMonate extends NumericSimilarityFunction<MonateValue> {

    private Integer globalMax = null;
    private Integer globalMin = null;

    @Override
    public Float calculateItemSim(CoraPropertyModel property, MonateValue a, MonateValue b) {
        //Globale min/max Werte
        globalMax = (globalMax == null)? getGlobalMaxValue(property, MonateValue.class).getValue() : globalMax;
        globalMin = (globalMin == null)? getGlobalMinValue(property, MonateValue.class).getValue() : globalMin;
